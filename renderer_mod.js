...
let settings = {
  ...
  showLevelBadge: true, // 添加是否显示层级标记的设置
  background: {
    type: 'none',
    value: '',
    opacity: 100,
...
async function loadSettings() {
  try {
    ...
    // 应用显示层级标记设置
    if (showLevelBadgeToggle) {
      showLevelBadgeToggle.checked = settings.showLevelBadge !== false; // 默认为true
    }
...
async function toggleShowLevelBadge() {
  ...
  try {
    ...
    console.error('更新显示层级标记设置失败:', error);
  }
}
