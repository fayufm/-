...
let settings = {
  ...
  showLevelBadge: true, // 添加显示层级标志设置，默认显示
  background: {
    type: 'none',
    value: '',
    opacity: 100,
...
addParentBtn.addEventListener('click', handleAddParentButtonClick);
  ...
  // 内存档按钮事件
  const memoryArchiveBtn = document.getElementById('memory-archive-btn');
  if (memoryArchiveBtn) {
    memoryArchiveBtn.addEventListener('click', openMemoryArchive);
  }
  ...
  // 在initApp函数中，添加对显示层级标志开关的监听
  const showLevelBadgeToggle = document.getElementById('show-level-badge-toggle');
  if (showLevelBadgeToggle) {
    showLevelBadgeToggle.checked = settings.showLevelBadge;
    showLevelBadgeToggle.addEventListener('change', toggleShowLevelBadge);
  }
}
...
function renderNotesList(notes) {
  ...
  const renderNote = (note, container, level = 1, parentClass = '') => {
    ...
    // 添加级别标志，根据设置决定是否显示
    const levelBadge = settings.showLevelBadge ? `<span class=\"level-badge\">${level}级</span>` : '';
...
function showAddNoteDialog() {
  ...
  delete noteTitleInput.dataset.genre; // 清除文体类型，避免与快捷添加功能冲突
...
function hideAddNoteDialog() {
  ...
  isAddingSubnote = false;
  delete noteTitleInput.dataset.genre; // 清除文体类型，防止影响下次添加
...
// 添加显示层级标志的切换函数
async function toggleShowLevelBadge() {
  const showLevelBadgeToggle = document.getElementById('show-level-badge-toggle');
  settings.showLevelBadge = showLevelBadgeToggle.checked;
  
  try {
    await ipcRenderer.invoke('update-settings', settings);
    // 重新加载笔记列表以应用新设置
    await loadNotes();
    showToast(settings.showLevelBadge ? '已开启层级标志显示' : '已隐藏层级标志显示');
  } catch (error) {
    console.error('更新层级标志设置失败:', error);
  }
}
...
async function openMemoryArchive() {
  try {
    hideSettingsDialog();
    const result = await ipcRenderer.invoke('open-memory-archive');
    
    if (result.success) {
      showToast('已打开内存档文件夹');
    } else {
      showToast('打开内存档失败: ' + (result.error || '未知错误'));
    }
  } catch (error) {
